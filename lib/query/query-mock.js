...
class Query {
  ...
  // Allow the tester to specify what the query should return at the end of the assertion chain
  returns(err, data) {
...
for (const chainableFunction of chainableQueryFunctions) {
  ...
  Query.prototype[chainableFunction.name] = function (...parameters) {
    ...
    this.executingChain.chain.push({ func: chainableFunction.name, matchers: callbackUtil.getParametersWithoutCallback(parameters) });
